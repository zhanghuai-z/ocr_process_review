...
4. fallback 数据进入 proof 时必须能被识别。估算或不可用几何不产生可裁切 occurrence；明确标源且带原始 bbox 的 word observation 以一个完整 word occurrence 进入 proof，不拆出伪字符框。
...
| EngCut 字符框冲突 PP word 降级 | `micro_recblock._engcut_route_line_text_and_chars()` | EngCut native group 内字符 bbox 发生重叠，且该 segment 只有一个 PP 拉丁 word token 并已有唯一前景所有权生成的 route bbox | 聚合同一 token 的连续 native groups；以 PP token 文本和 route 前景闭合 bbox 只生成一次 `source="ppocrv6:latin_token_text_route_foreground_geometry"` 的 word atom，聚合后的 native 文本和 bbox 只保存为外部候选；PP 原始 word bbox 仍只是路由 observation | atom 来源、`latin_token_geometry_fallback` review flag 和 `RunStats.latin_token_geometry_fallbacks` 可见；Proof 将它投影为一个可裁切、可整段编辑的 word occurrence | token-local PP 文本 observation 与 route 前景几何共同接管；无唯一 token、无前景闭合几何或 group 所有权穿插时阻断，不保留损坏字符框，也不扩张成整行回填 |
