...
| 块大类 | `Block.block_type` / `set_layout_block_type()` / `app.adapters.paddle.map_paddle_label_to_block_type()` | Paddle 原始 label 直接判断、各模块直接写 `block.block_type`、未知 label substring 猜测 | UI 和导出看大类；运行时改写必须经 layout_block_state helper；Paddle adapter 只做显式 label 映射，未知 label 保持 `UNKNOWN` 或按当前 block_type 默认路径处理，不能因包含 `formula/table/title` 等片段静默改结构类型。 |
...
| OCR 文本观察 | `app.models.ocr_text_observation` helpers + `ocr_text_observation_store`：`create_ocr_text_line()`、`line_ocr_text_observation()`、`line_ocr_review_flags()`、`line_has_ocr_review_flag()`；显示/OCR 原文读取走 `line_text_contract()` / `proof_ocr_text()` | 各 OCR producer 直接 `Line(text=..., ocr_text=...)`、业务层直接判断 `Line.text` / `Line.ocr_text` / `Line.review_flags`、旧 `line_ocr_text(line)` object helper | `text/ocr_text/confidence/review_flags` 是 OCR 文本观察；运行时事实已进入 text observation store，同时投影到 `Line` 供当前 UI/存储读取；旧 object 文本读 helper 已删除。 |
...
- `PaddleBinding`：仍是 active `Block` 上的 typed 绑定状态；允许字段已冻结，`from_dict()` 遇到未知字段会拒绝，避免重新长成 typed `app_payload`。
