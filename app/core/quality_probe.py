...
3. **混淆发生在切图 / 字形层** — proof text **绝不被污染**。混淆的是 VProof
...
``Probe``：
- ``key`` → 文档中一个真实位置，该位置 proof text[char_index] 上的字就是
  ``true_char``（OCR 原字 / 用户视为"对"的字）。
- ``fake_char`` → 该 true_char 的视觉近形字（且在文档别处出现过）；它只在
  VProof "显示空间" 被注入到该位置，proof text 本身**永不被 fake_char 改写**——这样
...
class Probe:
    ...
    - ``key`` 指向**文档真实位置**，该位置 proof text 上的字符就是 true_char。
...
def count_existing_cjk_crop_chars(line: Line) -> int:
    return sum(
        1
        for idx, ch in enumerate(proof_display_text(line))
    ...
    def sample(self, project: OcrProject) -> ProbeStore:
        ...
        for cand in pool:
            ...
            if per_line_count.get(line_key, 0) >= self.cfg.max_per_line:
                continue
            # Round 18 起重定义：
            #   probe.true_char = cand.char = 该位置 proof text 上的原字符（"正确字"）
            #   probe.fake_char = CONFUSION_MAP[cand.char] ∩ chars_in_doc \ {cand.char}
            #                      （即"显示空间里要注入的假象字"，且文档中实际存在）
            # proof text 永远保持 true_char；displayed_text 才把该位置渲染成 fake_char。
...
def detect_corrections(
    ...
    """以 proof text 为锚扫一遍 store 内所有 pending probe：

    若 ``proof_display_text(line)[probe.key.char_index]`` 已不再是投放时的 true_char（
