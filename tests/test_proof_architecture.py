...
def test_ocr_proof_projection_writes_chars_by_line_uid():
    source = Path("app/core/ocr_proof_projection.py").read_text(encoding="utf-8")
...
def test_proof_crop_service_writes_chars_by_line_uid():
    source = Path("app/services/proof_crop_service.py").read_text(encoding="utf-8")
...
def test_ocr_observation_geometry_writes_stay_at_observation_boundaries():
    allowed_by_pattern = {
        ...
        r"\bline\.chars\[[^\n]+\]\s*=(?!=)": {Path("app/engines/hanwang/micro_recblock.py")},
    }
    offenders: list[str] = []
    for path in sorted(APP_DIR.rglob("*.py")):
        source = path.read_text(encoding="utf-8")
...
def test_ocr_panel_tree_state_uses_uid_payloads_not_model_objects():
    source = Path("app/ui/recognize/ocr_panel.py").read_text(encoding="utf-8")

    assert "_tree_payload(\"page\"" in source
    assert "_tree_payload(\"block\"" in source
    assert "_tree_payload(\"line\"" in source
    assert "setData(0, Qt.ItemDataRole.UserRole, page)" not in source
    assert "setData(0, Qt.ItemDataRole.UserRole, block)" not in source
    assert "setData(0, Qt.ItemDataRole.UserRole, line)" not in source
    assert "isinstance(obj, Block)" not in source
    assert "isinstance(obj, Page" not in source
...
def test_layout_panel_user_edits_go_through_layout_edit_service():
    ...
    for command_name in ("update_geometry", "delete_block", "change_kind"):
        command_source = _function_source(edit_service_source, command_name)
...
def test_proof_readers_use_ocr_observation_store_not_block_lines_projection():
    ...
    proof_line_facts_source = Path("app/core/proof_line_facts.py").read_text(encoding="utf-8")
    assert "block_ocr_line_observations_by_uid" in proof_line_facts_source
    assert "block_ocr_line_observations(" not in proof_line_facts_source
    ...
    quality_probe_source = Path("app/core/quality_probe.py").read_text(encoding="utf-8")
    assert "block_ocr_line_observations_by_uid" in quality_probe_source
    assert "block_has_ocr_line_observations" not in quality_probe_source
    proof_crop_source = Path("app/services/proof_crop_service.py").read_text(encoding="utf-8")
    assert "block_ocr_line_observations_by_uid" in proof_crop_source
    assert "block_ocr_line_observations(" not in proof_crop_source
