...
def _seed_page_ocr_observations(page):
    ...
    snapshot = layout_snapshot_for_page(page)
    if snapshot is None or snapshot.source_engine in {"test_fixture", "test_seed"}:
...
def test_bbox_tools():
    ...
    assert shifted_chars[1].bbox_source == "ocr"
...
def test_project_store_upsert_rejects_foreign_parent_rowids():
    ...
    try:
        ...
        assert reloaded2_char.id == original_p2_ids[3]
...
def test_ocr_pipeline_offsets_crop_relative_boxes():
    ...
    try:
        ...
        assert chars[2].bbox == BBox(160, 70, 20, 18)
        assert chars[3].bbox == BBox(180, 70, 20, 18)
    finally:
        os.unlink(img_path)
...
def test_ocr_pipeline_prefers_engine_char_boxes_and_only_falls_back_for_missing_chars():
    ...
    try:
        ...
        assert chars[1].bbox_source == "ocr"
        assert chars[2].bbox_source == "fallback"
        assert chars[2].bbox_granularity == "fallback"
    finally:
        os.unlink(img_path)
...
def test_ocr_pipeline_process_block_normalizes_proof_geometry():
    ...
    try:
        ...
        assert [char.bbox_granularity for char in chars] == ["fallback", "fallback"]
    finally:
        os.unlink(img_path)
...
def test_ocr_pipeline_preserves_hanwang_crop_lines_and_chars():
    ...
    try:
        ...
        assert all(char.bbox_source.startswith("hanwang:") for char in chars)
    finally:
        os.unlink(img_path)
...
def test_project_store_update_proof_lines_does_not_move_foreign_char_uid():
    ...
    try:
        ...
        assert [char.char for char in loaded_line2_chars] == ["乙"]
        assert loaded_line2_chars[0].uid == line2_uid
    finally:
        if store is not None:
            store.close()
        try:
