...
1. **OCR 文本窗口也要体现假象**：displayed_text 把 fake_char 注入到显示空间，
   OCR 底文本保持 true_char，proof_display_text 是校对锚点。
2. **更正后回正确集合**：save_displayed_edit_result 反向写回时，proof_display_text[i] 变了就
...
def _plant_probe(line, page, block, true_ch, fake_ch, char_index):
    assert proof_display_text(line)[char_index] == true_ch, "测试前提：proof text 在 idx 上必须是 true_char"
    store = qp.ProbeStore()
    probe = qp.Probe(
...
def test_round18_displayed_text_no_active_store_returns_line_text():
    page, block, line = _make_pages()
    qp.reset_active_store()
    assert displayed_text(line, page, block) == proof_display_text(line)
...
def test_round18_save_displayed_edit_result_unchanged_keeps_probe_pending():
    ...
    assert proof_display_text(line) == "己已"
...
def test_round18_corrected_probe_position_appears_in_true_char_gallery():
    ...
    # proof_display_text 已是 "己已"——任何后续 CharIndexService.build(pages) 都会按
    # proof_display_text(line)[1]=="已" 把 idx=1 收入"已" gallery 集合，这就是"归位"语义。
    assert proof_display_text(line) == "己已"
    assert proof_display_text(line)[1] == "已"  # 文本锚点：query("已") 必然会命中此位置
...
def test_round18_detect_corrections_picks_up_bypass_path_edits():
    page, block, line = _make_pages()
    store, probe = _plant_probe(line, page, block, true_ch="已", fake_ch="己", char_index=1)
    # 模拟 batch-replace / 其它绕过 bridge 的路径：直接改 proof text
    set_line_proof_text(line, "己巳")
