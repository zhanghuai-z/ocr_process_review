...
_PARAMS: list[tuple] = [
    ...
    ("textDetUnclipRatio", "float", str(PADDLE_OCR_WORD_BOX_PARAMS["textDetUnclipRatio"]),
     "检测框扩张比例 (Vatti clipping algorithm)",
     "值越大框越宽松，有助于包住完整字符；过大会合并相邻行"),
    ("textDetLimitSideLen", "int", str(PADDLE_OCR_WORD_BOX_PARAMS["textDetLimitSideLen"]),
     "送入检测网络前的最长边缩放上限 (px)",
    ...
    ("returnWordBox", "bool", str(PADDLE_OCR_WORD_BOX_PARAMS["returnWordBox"]),
     "是否返回字/词级 bounding box (text_word_region)",
     "开启后 overall_ocr_res.text_word_region 有值；canvas 中橙色字框才会出现"),

    # ── 预处理 Preprocessing ──
    ("__cat__", "预处理 (Preprocessing)", "", "", ""),
    ("useDocOrientationClassify", "bool", str(PADDLE_COORD_STABILITY_FLAGS["useDocOrientationClassify"]),
     "文档方向分类（0°/90°/180°/270°自动旋转）",
     "共享请求链默认关闭以保持 bbox 坐标稳定；开启会让服务端旋转后坐标语义变化"),
    ("useDocUnwarping", "bool", "False",
     "文档去扭曲 (TPS/STN unwarp)",
     "对扫描件有用；开启会改变坐标空间（坐标仍在处理后图上）"),
    ("useTextlineOrientation", "bool", str(PADDLE_COORD_STABILITY_FLAGS["useTextlineOrientation"]),
     "文本行方向分类（横排/竖排）",
     "共享请求链默认关闭以保持 bbox 坐标稳定；开启可能改变行方向处理后的坐标语义"),
...
_MATRIX_SPECS: dict[str, tuple[object, str, str]] = {
    "returnWordBox": (PADDLE_OCR_WORD_BOX_PARAMS["returnWordBox"], "PP-OCRv5, PP-StructureV3", "请求 text_word/text_word_region；VL family 不支持真实 word/char bbox。"),
    "useDocOrientationClassify": (PADDLE_COORD_STABILITY_FLAGS["useDocOrientationClassify"], "全部官方 profile", "坐标稳定保护项；共享请求链默认关闭，避免服务端旋转后 bbox 与原图漂移。"),
    "useDocUnwarping": (PADDLE_COORD_STABILITY_FLAGS["useDocUnwarping"], "全部官方 profile", "坐标稳定保护项；开启会改变处理后图坐标空间。"),
    "useTextlineOrientation": (PADDLE_COORD_STABILITY_FLAGS["useTextlineOrientation"], "全部官方 profile", "坐标稳定保护项；共享请求链默认关闭，避免行方向分类造成坐标语义变化。"),
    "textDetThresh": (PADDLE_OCR_WORD_BOX_PARAMS["textDetThresh"], "PP-OCRv5, PP-StructureV3", "文本检测阈值，仅 OCR detector/recognizer family 接收。"),
    "textDetBoxThresh": (PADDLE_OCR_WORD_BOX_PARAMS["textDetBoxThresh"], "PP-OCRv5, PP-StructureV3", "检测框分数阈值，仅 OCR detector/recognizer family 接收。"),
    "textDetUnclipRatio": (PADDLE_OCR_WORD_BOX_PARAMS["textDetUnclipRatio"], "PP-OCRv5, PP-StructureV3", "检测框扩张比例，仅 OCR detector/recognizer family 接收。"),
    "textDetLimitSideLen": (PADDLE_OCR_WORD_BOX_PARAMS["textDetLimitSideLen"], "PP-OCRv5, PP-StructureV3", "检测前最长边限制，仅 OCR detector/recognizer family 接收。"),
    "textDetLimitType": (PADDLE_OCR_WORD_BOX_PARAMS["textDetLimitType"], "PP-OCRv5, PP-StructureV3", "检测缩放策略，仅 OCR detector/recognizer family 接收。"),
    "textRecScoreThresh": (PADDLE_OCR_WORD_BOX_PARAMS["textRecScoreThresh"], "PP-OCRv5, PP-StructureV3", "识别分数过滤阈值，仅 OCR detector/recognizer family 接收。"),
...
def build_paddle_param_matrix(profile: str = "pp-structurev3", endpoint_url: str | None = None) -> tuple[list[ParamMatrixRow], dict]:
    ...
    for name, (default, models, note) in _MATRIX_SPECS.items():
        ...
        current = options[name] if sent else "unsupported"
        if sent:
